...
app.layout = html.Div([
    html.H1("FailProof LLM - Failure Analysis Dashboard", style={'textAlign': 'center'}),

    dcc.Graph(figure=fig_pie),

    dcc.Graph(figure=fig_bar),

    html.H2("Test Case Details"),

    dash_table.DataTable(
        id='table',
        columns=[
            {"name": "Prompt", "id": "prompt", "presentation": "markdown"},
            {"name": "Latency (s)", "id": "latency_sec"},
        ...
        style_data_conditional=[
            {
                'if': {'filter_query': '{failure_type} = "Success"'},
                'backgroundColor': 'rgb(198, 239, 206)',
            },
            {
                'if': {'filter_query': '{failure_type} = "Incorrect Output"'},
                'backgroundColor': 'rgb(255, 199, 206)',
            },
            {
                'if': {'filter_query': '{failure_type} = "Refusal"'},
                'backgroundColor': 'rgb(255, 235, 156)',
            },
            {
                'if': {'filter_query': '{failure_type} = "Policy Violation"'},
                'backgroundColor': 'rgb(255, 179, 179)',
            },
            {
                'if': {'filter_query': '{failure_type} = "Crash/Error"'},
